package com.peru.ncov2019.ml.sqs.service.perupu;
...
/**
 * Clase para buscar Centros de Salud
 *
 */

@CommonsLog
@Service("PERUPU0001")
public class PERUPU0001 implements TargetService{
    ...
    public ModelAndView serviceCall(Map<String, Object> vo, HttpServletRequest request)  throws Exception {
           ...
           try{
                  String msg = MlUtil.emptyValidation(
                               vo,
                               new String[][] {{"CURRENT_X","Coordenadas de longitud"},
                                      		   {"CURRENT_Y","Coordenadas de latitud"}}
                               );
                  vo = MlUtil.mappingSn(vo, ScmDao);
                  if(!"".equals(msg)){
                        resCdMapper.setMsg(mav , "110");
                  }
                  else {
                	  // Búsqueda de Centros de Salud
                      List<Map<String,Object>> list = ScmDao.selectList("PERU_USR.selectMdlcnstList",vo);
